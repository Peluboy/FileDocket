...
This explains how to hand FileDocket to other people cleanly. It's separate from
...
- **The macOS `.app` is fully self-contained.** Auto-Organize uses a CLI binary embedded *inside*
  the bundle, so the app works alone in `/Applications` (no companion `FileDocket-Mac` file
  required).
...
- **.dmg**: `hdiutil create -volname "FileDocket" -srcfolder <folder> -ov -format UDZO FileDocket.dmg`
  then `codesign` + notarize + `xcrun stapler staple FileDocket.dmg`
